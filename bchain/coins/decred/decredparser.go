...
func NewDecredParser(params *chaincfg.Params, c *Configuration) *DecredParser {
	p := &DecredParser{
...
// the regression test Decred network, the test Bitcoin network and
// the simulation test Decred network, in this order
...
func (p *DecredParser) GetScriptFromAddrDesc(addrDesc bchain.AddressDescriptor) ([]byte, error) {
	return addrDesc, nil
}

// addressToOutputScript converts bitcoin address to ScriptPubKey
func (p *DecredParser) addressToOutputScript(address string) ([]byte, error) {
...
func (p *DecredParser) TryParseOPReturn(script []byte) string {
...
func (p *DecredParser) tryParseOmni(data []byte) string {
...
func (p *DecredParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *DecredParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
